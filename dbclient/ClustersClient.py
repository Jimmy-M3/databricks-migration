    ...
    def import_cluster_policies(self, log_file='cluster_policies.log', acl_log_file='acl_cluster_policies.log'):
        ...
        if os.path.exists(policies_log):
            with open(policies_log, 'r', encoding="utf-8") as policy_fp:
                skip_policies = ['Power User Compute','Personal Compute','Shared Compute','Job Compute']

                for p in policy_fp:
                    policy_conf = json.loads(p)
                    if policy_conf['name'] in skip_policies:
                        continue
            ...
            with open(acl_policies_log, 'r', encoding="utf-8") as acl_fp:
                ...
                for x in acl_fp:
                    ...
                    single_policy_detail = self.get('/policies/clusters/get',json_params=dict(policy_id=policy_id))
                    acl_create_args = dict(
                        name = single_policy_detail['name'],
                        definition = single_policy_detail['definition'],
                        description = single_policy_detail.get('description',None),
                        policy_family_id = single_policy_detail.get('policy_family_id',None)
                    )
                    # api = f'/permissions/cluster-policies/{policy_id}'
                    api = '/policies/clusters/create'
                    resp = self.post(api, acl_create_args)
    ...
    def log_cluster_policies(self, log_file='cluster_policies.log', acl_log_file='acl_cluster_policies.log'):
        ...
        with open(policies_log, 'w', encoding="utf-8") as fp:
            ...
            for x in policies_list:
                # if x.get('name') in skip_policies:
                #     continue
