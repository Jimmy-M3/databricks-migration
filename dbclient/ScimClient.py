    ...
    def add_username_to_group(self, group_json):
        ...
        for m in members:
            ...
            if self.is_member_a_user(m):
                ...
                sp_infos = self.get('/preview/scim/v2/ServicePrincipals/{id}'.format(id=m['value']))
                m['application_id'] = sp_infos['applicationId']
    ...
    def import_groups(self, group_dir, current_user_ids, error_logger):
        ...
        for x in groups:
            if not checkpoint_groups_set.contains(x):
                ...
                group_resp = self.post('/preview/scim/v2/Groups', create_args) #old
                # group_resp = self.post('/accounts/20c26866-115e-4e40-a202-a0f035943ad5/scim/v2/Groups', create_args) #new
                ...
                if members:
                    ...
                    for m in members:
                        if self.is_user(m):
                            ...
                            # this_group_detail = self.get('/preview/scim/v2/Groups?filter=displayName%20eq%20%22{group}%22'.format(group=group_name))
                            # this_group_id = this_group_detail.get('Resources')[0]['id']
                            this_group_id = current_group_ids.get(group_name)
                                ...
                                "groups": [
                                    {
                                        ...
                                        "type":"direct",
                                        "value": this_group_id,
                                        "$ref": "Groups/{0}".format(this_group_id)
                                    }
                                ],
                                "entitlements": [
                                    {
                            ...
                            print("="*30)
                            print(payload_service_principal)
                            add_azure_spns = self.post(
                                '/preview/scim/v2/ServicePrincipals', payload_service_principal)
                            if add_azure_spns.get('status') == '409':
                                print("ServicePrincipal already exists!")
                                sp_info = self.get('/preview/scim/v2/ServicePrincipals?filter=displayName%20eq%20%22{sp_name}%22'.format(sp_name= m['display'])).get('Resources')[0]
                                sp_id = sp_info.get('id')
                                sp_groups = sp_info.get('groups',[])
                                sp_groups.append(
                                    {"type":"direct","value": this_group_id,"$ref": "Groups/{0}".format(this_group_id)}
                                )
                                self.put('/preview/scim/v2/ServicePrincipals/{id}'.format(id=sp_id), {'applicationId':m['application_id'],'groups': sp_groups})
                                print({'groups': sp_groups})
