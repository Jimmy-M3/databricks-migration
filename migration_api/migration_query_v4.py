...
#query_path = os.path.join(script_dir, 'log\\query_path.log')
...
def create_query(url_target, access_token_target, query_data,type,update_mask =None):
    ...
    return 
#end 构造创建dlt函数
def _acl_import_helper(databricks_domain,access_token,source_sqlwarehouse_ids,target_sqlwarehouse_ids,query_target_ids,_access_token_target,_databricks_domain_target,json_data):
    ...
    for id in query_target_id:
        ...
        if parent_path.replace('anker.com','anker-in.com') == parent_target_path:
            query_id = id       
    # for query_data in query_target_ids:
    #     if query_data.get('display_name') == display_name:
    ...
    query_path_json={"display_name":display_name,"parent_path":parent_path}
    with open(query_path, "a", newline="", encoding='utf-8' ) as fp:
...
def call_migration_query(source_domain,source_token,domain_target,token_target):
    ...
    with open(file_path, 'r', encoding='utf-8') as fp: 
        ...
        with ThreadPoolExecutor(max_workers=10) as executor:
                futures = [executor.submit(_acl_import_helper,databricks_domain,access_token,source_sqlwarehouse_ids,target_sqlwarehouse_ids,query_target_ids,_access_token_target,_databricks_domain_target,json_data) for json_data in fp]
